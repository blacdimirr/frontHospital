...
class Interconsultation extends Admin_Controller
{
    ...
    {
        $this->session->set_userdata('top_menu', 'interconsultation');
        ...
        $this->load->view('admin/interconsultation/index', $data);
        $this->load->view('layout/footer');
    }

    public function updateOrdenMedica($id = null)
    {
        if(empty($id)){
        ...
        $up = $this->appointment_model->updateOrdenMedica($data);
    ...
    {
        $dt_response = $this->appointment_model->getAllinterconsultationRecord();
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action .="<a href='#'  class='btn btn-default btn-xs' data-toggle='tooltip'  onclick='visitado(" . $value->id .")' data-original-title=''><i class='fa fa-calendar'></i></a>";
                ...
                $action .= "</div>";
                $first_action = "<a  href='". base_url()."admin/patient/profile/" . $value->pid . "' data-toggle='tooltip' title=''>";
     ...
     public function visitado(){
        ...
        $case_reference_id = $this->input->post('_id');
        ...
        $data = array(
            "id" => $case_reference_id,
            "is_visitado"         => 1,
        );
        $this->appointment_model->updateinterconsultation($data);
