...
$(document).on('click', '.addprescription', function() {
    ...
    $.ajax({
        ...
        type: "POST",
        beforeSend: function() {
            // Aquí podrías agregar algún tipo de indicador de carga
        },
        success: function(res) {
            $('#edit_prescription_title').html('<?php echo $this->lang->line('add_prescription'); ?>');
            var $modalBody = $('#add_prescription .modal-body');
            $modalBody.html(res.page);

            // Inicializar Select2 para todos los select en el modal
            $modalBody.find('.select2').select2();
            $modalBody.find('.multiselect2').select2({
                placeholder: 'Select',
                allowClear: false,
                minimumResultsForSearch: 2
            });
        ...
        complete: function() {
            $("#compose-textareasadd, #compose-textareanewadd").each(function() {
                $(this).wysihtml5({
                    toolbar: {
                        "image": false
                    }
