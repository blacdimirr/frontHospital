...
class Issueitem extends Admin_Controller
{
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($value->is_returned == 1) {
                    $status = "<span  class='label label-danger item_remove'  data-item='" . $value->id . "' data-category='" . $value->item_category . "' data-item_name='" . $value->item_name . "' data-quantity='" . $value->quantity . "' data-sku='" . $value->sku . "' data-toggle='modal' data-target='#confirm-delete' title=''>" . $this->lang->line('click_to_return') . "</span>";
                } else {
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $sku = $this->input->post('sku');
            $quantity = $this->input->post('quantity_product') - $this->input->post('quantity') ;
            $up = $this->updateProduct($sku, $quantity);
