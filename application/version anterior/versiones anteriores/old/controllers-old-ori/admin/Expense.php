...
class Expense extends Admin_Controller
{
            ...
            foreach ($dt_response->data as $key => $value) {
                ...
                if (!empty($fields)) {
                    foreach ($fields as $fields_key => $fields_value) {
                        if (($fields_value->id == 7) != ($value->{"$fields_value->name"} == 'PAGADO') ){
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($custom_field_post[7] == 'PAGADO'){
                // $id_income = $this->input->post('income_id');
                // $income  = $this->income_model->get($id_income);

                print_r(empty($_FILES['documents_other']['name']));
                die();

                if ((($this->input->post('amount') > $this->input->post('amount_income')) || empty($_FILES['documents_other']['name'])) ) {
                    $array_custom     = array(
                        'belong_table_id' => $id,
                        'custom_field_id' => 7,
                        'field_value'     => 'PENDIENTE',
