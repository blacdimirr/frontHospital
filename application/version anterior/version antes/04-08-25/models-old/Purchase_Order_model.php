...
class Purchase_Order_model extends MY_Model
{
    ...
    {
         $this->datatables
            ->select('purchase_order.*,item_supplier.item_supplier,purchase_status.purchase_status_name')
            ->join('item_supplier', 'item_supplier.id = purchase_order.supplier_id')
            ->join('purchase_status', 'purchase_status.id = purchase_order.purchase_status')            
            ->from('purchase_order');
    ...
    {
         $this->datatables
            ->select('purchase_order_item_store.*,item_store.item_store') 
            ->join('item_store', 'item_store.id = purchase_order_item_store.item_store_id')
            ->from('purchase_order_item_store');

        if ($id != null) {
            $this->db->where('purchase_order_item_store.purchase_order_id', $id);
        } else {
            $this->db->order_by('purchase_order_item_store.id', 'ASC');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->result_array();
    ...
    public function insertRecord($data,$delete = null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        if ($delete !== NULL) {
            $this->db->where('purchase_order_id', $data[0]['purchase_order_id']);
            // $this->db->update('purchase_order', $data);
            $this->db->delete('purchase_order_item_store');
            $message = DELETE_RECORD_CONSTANT . " Where purchase_order_item_store id " . $data[0]['purchase_order_id'];
            $action = "Delete";
            $record_id = $data[0]['purchase_order_id'];
            $this->log($message, $record_id, $action);

            // $this->db->insert('purchase_order_item_store', $data);
            // $insert_id = $this->db->insert_id();

            $insert_ids = array();
            $insert_id = 0;
            foreach ($data as $key => $value) {
                $this->db->insert('purchase_order_item_store', $value);
                $insert_id = $this->db->insert_id();
            ...
            $message = INSERT_RECORD_CONSTANT . " On purchase_order_item_store id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            // print_r($insert_ids);
            $this->log($message, $record_id, $action);
