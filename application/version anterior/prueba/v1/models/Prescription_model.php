...
class Prescription_model extends MY_Model
{
    ...
    public function add_hoja_ingreso($data)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        if (isset($data['id'])) {
            ...
            $this->db->update('hoja_ingreso', $data);
            $message = UPDATE_RECORD_CONSTANT . " On Ipd Prescription Test where Prescription id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            ...
            $this->db->insert('hoja_ingreso', $data);
    ...
    public function add_hoja_ingreso_is_hospitalized($data)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
    ...
    public function add_Interconsultation($data)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        if (isset($data['id'])) {
            ...
            $this->db->where('id', $data['id']);
            $this->db->update('formulario_interconsulta', $data);
            $message = UPDATE_RECORD_CONSTANT . " On Ipd Prescription Test where Prescription id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            ...
            $this->db->insert('formulario_interconsulta', $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On Ipd Prescription Test id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
    ...
    public function getPrescriptionByTable($id, $table_type)
    {
        ...
        if ($table_type == "ipd_prescription") {
            ...
            $query = $this->db->select("opd_details.*,patients.*,staff.name,staff.surname,staff.local_address,ipd_prescription_basic.ipd_id,ipd_prescription_basic.id as prescription_id,ipd_prescription_basic.date as presdate,ipd_prescription_basic.header_note,ipd_prescription_basic.footer_note,ipd_prescription_basic.finding_description,ipd_prescription_basic.is_finding_print,visit_details.id as visit_details_id,visit_details.opd_details_id");
    ...
    public function getPrescriptionById($prescription_id)
    {
        ...
        $this->db->where("ipd_prescription_basic.id", $prescription_id);
        $query = $this->db->get("ipd_prescription_basic");
    ...
    public function get_hoja_ingreso_by_ipd($ipd_id)
    {
        $query = $this->db->select("hoja_ingreso.*,patients.*,
        ipd_details.case_reference_id,
        hoja_ingreso.id as hoja_ingreso_id,        
        prescription_generate.name as generated_by_name,prescription_generate.surname as generated_by_surname,prescription_generate.employee_id as generated_by_employee_id,
        prescribe_by.name as prescribe_by_name,prescribe_by.surname as prescribe_by_surname,
        prescribe_by.employee_id as prescribe_by_employee_id,
        prescribe_by.employee_id as doctor_id");
        $this->db->join("ipd_details", "ipd_details.id = hoja_ingreso.ipd_id");
        $this->db->join("patients", "patients.id = ipd_details.patient_id");
        $this->db->join("staff as prescription_generate", "prescription_generate.id = hoja_ingreso.generated_by");
        $this->db->join("staff as prescribe_by", "prescribe_by.id = ipd_details.cons_doctor");

        $this->db->where("hoja_ingreso.ipd_id", $ipd_id);
        $query = $this->db->get("hoja_ingreso");
        return $query->row();
    }

    public function get_hoja_ingreso_by_ipd_details($ipd_id)
    {
        $query = $this->db->select("
            ...
            patients.known_allergies,
            patients.note AS patients_note,
            patients.is_ipd,
            patients.insurance_id,
            patients.insurance_validity,
            patients.is_dead,
            ...
            ipd_details.weight,
            ipd_details.pulse,
            ipd_details.temperature,
            ipd_details.respiration,
            ipd_details.bp,
            ipd_details.bed,
            ipd_details.bed_group_id,
            ipd_details.case_type,
            ipd_details.casualty,
            ipd_details.symptoms,
            ipd_details.known_allergies,
            ipd_details.patient_old,
            ipd_details.note AS ipd_details_note,
            ipd_details.refference,
            ipd_details.cons_doctor,
            ipd_details.organisation_id,
            ipd_details.credit_limit,
            ipd_details.payment_mode,
            ipd_details.date AS appointment_date,
            ipd_details.discharged,
            ipd_details.live_consult,
            ipd_details.generated_by
        ");
        $this->db->join("patients", "patients.id = ipd_details.patient_id");
        $this->db->where("ipd_details.id", $ipd_id);
    ...
    public function get_hoja_ingreso_by_ipd_details_2($ipd_id)
    {
        $query = $this->db->select("
            patients.patient_name,
            patients.dob,
    ...
    public function getPrescriptionByVisitHojaIngresoID($visitid)
    {
        $query = $this->db->select("hoja_ingreso.*,visit_details.id as visitid,visit_details.known_allergies as any_allergies,visit_details.weight,visit_details.height,visit_details.pulse,visit_details.temperature,visit_details.symptoms,visit_details.bp,
        patients.*,
        opd_details.case_reference_id,
        staff.name,staff.surname,staff.employee_id,staff.local_address,
        
        visit_details.opd_details_id,
        visit_details.id visit_id,
        visit_details.appointment_date,
        hoja_ingreso.id as hoja_ingreso_id,

        prescription_generate.name as generated_by_name,prescription_generate.surname as generated_by_surname,prescription_generate.employee_id as generated_by_employee_id,
        prescribe_by.name as prescribe_by_name,prescribe_by.surname as prescribe_by_surname,prescribe_by.employee_id as prescribe_by_employee_id, opd_details.id as opd_detail_id,staff.employee_id as doctor_id");
        $this->db->join("visit_details", "visit_details.id = hoja_ingreso.visit_details_id","left");
        // $this->db->join("hoja_ingreso", "visit_details.id = hoja_ingreso.visit_details_id","left");
        $this->db->join("opd_details", "opd_details.id = visit_details.opd_details_id");
        $this->db->join("patients", "patients.id = opd_details.patient_id");
        $this->db->join('blood_bank_products', 'blood_bank_products.id = patients.blood_bank_product_id',"left");
        $this->db->join("staff", "staff.id = visit_details.cons_doctor");
        $this->db->join("staff as prescription_generate", "prescription_generate.id = hoja_ingreso.generated_by");
        $this->db->join("staff as prescribe_by", "prescribe_by.id = hoja_ingreso.prescribe_by");

        $this->db->where("hoja_ingreso.visit_details_id", $visitid);
        $query = $this->db->get("hoja_ingreso");
    ...
    public function getPrescriptionByVisitInterconsultaID($visitid)
    {
        $query = $this->db->select("formulario_interconsulta.*,visit_details.id as visitid,visit_details.known_allergies as any_allergies,visit_details.weight,visit_details.height,visit_details.pulse,visit_details.temperature,visit_details.symptoms,visit_details.bp,
        patients.*,
        opd_details.case_reference_id,
        staff.name,staff.surname,staff.employee_id,staff.local_address,
        ...
        visit_details.id visit_id,
        visit_details.appointment_date,
        formulario_interconsulta.id as formulario_interconsulta_id,

        prescription_generate.name as generated_by_name,prescription_generate.surname as generated_by_surname,prescription_generate.employee_id as generated_by_employee_id,
        prescribe_by.name as prescribe_by_name,prescribe_by.surname as prescribe_by_surname,prescribe_by.employee_id as prescribe_by_employee_id, opd_details.id as opd_detail_id,staff.employee_id as doctor_id");
        $this->db->join("visit_details", "visit_details.id = formulario_interconsulta.visit_details_id","left");
        // $this->db->join("hoja_ingreso", "visit_details.id = hoja_ingreso.visit_details_id","left");
        $this->db->join("opd_details", "opd_details.id = visit_details.opd_details_id");
        $this->db->join("patients", "patients.id = opd_details.patient_id");
        $this->db->join('blood_bank_products', 'blood_bank_products.id = patients.blood_bank_product_id',"left");
        $this->db->join("staff", "staff.id = visit_details.cons_doctor");
        $this->db->join("staff as prescription_generate", "prescription_generate.id = formulario_interconsulta.generated_by");
        $this->db->join("staff as prescribe_by", "prescribe_by.id = formulario_interconsulta.prescribe_by");

        $this->db->where("formulario_interconsulta.visit_details_id", $visitid);
        $query = $this->db->get("formulario_interconsulta");
