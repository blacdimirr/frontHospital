...
class Itemissue_model extends MY_Model
{
    ...
    {
        $query = $this->db->where("item_issue_id", $item_issue_id)->where("is_returned", 0)
            ->select('item_issue_details.*,item_store.item_store as item_store_name,
            ( 
     ...
     public function get_ordenes_compra_detalle($orden_compra_id)
    {
        $query = $this->db->where("ordenes_compra_detalle.orden_compra_id", $orden_compra_id)
            ->select('ordenes_compra_detalle.*,ordenes_compras.suplidores, ordenes_compras.orden_compra,ordenes_compras.note,ordenes_compras.received_date,staff.name as staff_name,staff.surname as staff_surname')
