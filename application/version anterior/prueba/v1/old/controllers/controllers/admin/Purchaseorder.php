...
class Purchaseorder extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'purchase_order/index');
        // purchase_order
        $data['title']      = $this->lang->line('add_purchase_order');
        ...
        $expense_result      = $this->purchase_order_model->get(); 
        ...
        $data['itemSupplierList'] = $itemSupplier;
        ...
        $this->load->view('admin/purchaseorder/purchaseOrderList', $data);
    ...
    {
        $dt_response = $this->purchase_order_model->getAllRecord();
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $row = array();
                //====================================
                $column_first = '<a href="#" data-toggle="popover" class="detail_popover">' . $this->customlib->YYYYMMDDTodateFormat($value->purchase_order_date). '</a>';
                ...
                if ($value->purchase_order_description == "") {
                    $column_first .= '<p class="text text-danger">' . $this->lang->line('no_description') . '</p>';
                } else {
                    $column_first .= '<p class="text text-info">' . $value->purchase_order_description . '</p>';
                }
                ...
                $row[] = $column_first;
                $row[] = $value->reference_number;
                // $row[] = $this->customlib->YYYYMMDDTodateFormat($value->purchase_order_date);
                // $row[] = $this->customlib->YYYYMMDDTodateFormat($value->purchase_order_date);
                $row[] = substr($value->purchase_order_description,0,60);
                $row[] = $value->item_supplier;
                $row[] = $value->purchase_status_name;
                // $row[] = $value->total_expenditure;
                //====================
                // if (!empty($fields)) {
                //     foreach ($fields as $fields_key => $fields_value) {
    ...
    {
        $data['title']      = $this->lang->line('add_purchase_order');
        $data['title_list'] = $this->lang->line('purchase_order_list');
        ...
        $this->form_validation->set_rules('item_store_id', $this->lang->line('products'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data   = array(
                ...
                'purchase_order_date'         => $this->customlib->dateFormatToYYYYMMDD($exdate),
                'payment_date'         => $this->customlib->dateFormatToYYYYMMDD($payment_date),
                'total_expenditure'       => $this->input->post('amount'),
                'reference_number'   => $this->input->post('reference_number'),
                'purchase_order_description'         => $this->input->post('description'),
                'supplier_id'         => $this->input->post('supplier_id'),

                'purchase_status'         => $this->input->post('supplier_id'),
            ...
            $row_costs = $this->input->post('row_costs');
            $row_quantitys = $this->input->post('row_quantitys');
            $data_child = array();
            
            $insert_id = $this->purchase_order_model->add($data);

            foreach ($rows as $key => $value) {
                 $data_temp = array(
                     'purchase_order_id' => $insert_id,
                     'item_store_id' => $rows[$key],
                     'amount' => $row_quantitys[$key],
                     'cost' => $row_costs[$key],
                 );
                $data_child[] = $data_temp;
            }

            $insert_ids = $this->purchase_order_model->insertRecord($data_child,1);
            ...
            if (isset($_FILES["documents"]) && !empty($_FILES['documents']['name'])) {
                ...
                move_uploaded_file($_FILES["documents"]["tmp_name"], "./uploads/hospital_purchase_order/" . $img_name);
                $data_img = array('id' => $insert_id, 'documents' => 'uploads/hospital_purchase_order/' . $img_name);
                $this->purchase_order_model->add($data_img);
            }
    ...
    public function get_date($value){
        return $this->customlib->YYYYMMDDTodateFormat($value);
    }
    ...
    public function view($id)
    {
        ...
        $expense         = $this->purchase_order_model->get($id);
    ...
    public function delete($id)
    {
        ...
        $data['title'] = $this->lang->line('fees_master_list');
        $this->purchase_order_model->remove($id);
    ...
    public function getDataByid($id)
    {
        ...
        // set_value('purchase_order_description', $expense['purchase_order_description']);
        ...
        $data['expense']['payment_date'] = $this->customlib->YYYYMMDDTodateFormat($data['expense']['payment_date']);
        // print_r($this->customlib->YYYYMMDDTodateFormat($data['expense']['payment_date']));
        // die();
        ...
        $purchase_order_details         = $this->purchase_order_model->get_details($id);
        $data['purchase_order_details'] = $purchase_order_details;
        
        // print_r($data['purchase_order_details']);
        // die();
        ...
        $this->load->view('admin/purchaseorder/editPurchaseOrderModal', $data);
    }
    ...
    public function getDataByidForStatus($id)
    {
        ...
        $expnseHead          = $this->expensehead_model->get();
        $data['expheadlist'] = $expnseHead;
        $this->load->view('admin/purchaseorder/editPurchaseOrderModalStatus', $data);
    }
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["documents"]) && !empty($_FILES['documents']['name'])) {
                ...
                move_uploaded_file($_FILES["documents"]["tmp_name"], "./uploads/hospital_purchase_order/" . $img_name);
                $data_img = array('id' => $id, 'documents' => 'uploads/hospital_purchase_order/' . $img_name);
                $this->purchase_order_model->add($data_img);
            }
            ...
            if (isset($_FILES["documents_other"]) && !empty($_FILES['documents_other']['name'])) {
                ...
                move_uploaded_file($_FILES["documents_other"]["tmp_name"], "./uploads/hospital_purchase_order/" . $img_name);
                $data_img = array('id' => $id, 'documents_other' => 'uploads/hospital_purchase_order/' . $img_name);
                $this->purchase_order_model->add($data_img);
    ...
    public function edit_status($id)
    {
        ...
        // print_r($this->input->post("search_type"));
        // die();

        $this->form_validation->set_rules('search_type', $this->lang->line('status'), 'trim|required|xss_clean');
    ...
    public function expensegroup()
    {
        ...
        $result              = $this->purchase_order_model->searchexpensegroup($search_type, $head_id);
    ...
    public function dtexpensegroupreport()
    {
        ...
        $result = $this->purchase_order_model->getexpensereport($start_date, $end_date, $head_id);
